// Unit tests for format.js pure functions (Hail-Mary M11 dashboard).
...
test("formatAxisTick: passes through empty/falsy input unchanged", () => {
  assert.equal(formatAxisTick(""), "");
  assert.equal(formatAxisTick(null), "");
});

test("formatAxisTick: shortens an ISO timestamp to MM-DD HH:MM", () => {
  assert.equal(formatAxisTick("2026-08-21T10:05:30Z"), "08-21 10:05");
});

test("formatAxisTick: falls back to the cleaned string when it has no date/time split", () => {
  assert.equal(formatAxisTick("not-a-timestamp"), "not-a-timestamp");
});

// ---- pickTickIndices ----

test("pickTickIndices: empty series returns no ticks", () => {
  assert.deepEqual(pickTickIndices(0), []);
});

test("pickTickIndices: fewer points than the max label count returns every index", () => {
  assert.deepEqual(pickTickIndices(3, 6), [0, 1, 2]);
});

test("pickTickIndices: always includes the first and last index", () => {
  const ticks = pickTickIndices(24, 6);
  assert.equal(ticks[0], 0);
  assert.equal(ticks[ticks.length - 1], 23);
  assert.ok(ticks.length <= 6);
});

test("pickTickIndices: never returns more than maxLabels indices", () => {
...
test("formatRelativeTime: null/falsy input returns empty string, never throws", () => {
  assert.equal(formatRelativeTime(null), "");
  assert.equal(formatRelativeTime(""), "");
});

test("formatRelativeTime: invalid date string returns empty string", () => {
  assert.equal(formatRelativeTime("not-a-date", Date.now()), "");
});

test("formatRelativeTime: under a minute is '방금 전'", () => {
  const now = Date.parse("2026-08-21T10:00:30Z");
  assert.equal(formatRelativeTime("2026-08-21T10:00:00Z", now), "방금 전");
});

test("formatRelativeTime: minutes/hours/days scale correctly", () => {
  const base = "2026-08-21T10:00:00Z";
  assert.equal(formatRelativeTime(base, Date.parse("2026-08-21T10:12:00Z")), "12분 전");
  assert.equal(formatRelativeTime(base, Date.parse("2026-08-21T13:00:00Z")), "3시간 전");
  assert.equal(formatRelativeTime(base, Date.parse("2026-08-24T10:00:00Z")), "3일 전");
});

test("formatRelativeTime: past a week falls back to the absolute date", () => {
  const result = formatRelativeTime("2026-08-01T10:00:00Z", Date.parse("2026-08-21T10:00:00Z"));
  assert.equal(result, "2026-08-01");
});

// ---- buildKpiSummary ----

test("buildKpiSummary: all sources missing -> every field null, not zero/invented", () => {
  const kpi = buildKpiSummary({
    occupancy: { hasData: false, total: 0, zones: [] },
    ablation: { n: 0, with_occ_available_count: 0 },
    openAnomalyCount: null,
    savingsCard: { hasData: false },
  });
...
test("buildKpiSummary: pulls each value from its already-fetched source", () => {
  const kpi = buildKpiSummary({
    occupancy: { hasData: true, total: 7, zones: [] },
    ablation: { n: 40, with_occ_available_count: 25, mae_improvement_pct: 18.4 },
    openAnomalyCount: 2,
    savingsCard: { hasData: true, raw: { saved_pct: 8.2 } },
  });
  assert.deepEqual(kpi, {
    totalOccupancy: 7,
    openAnomalyCount: 2,
